...
def mapping_other_post_with_customer_frame(sender, instance, created, **kwargs):
    if created:
        ...
        for customer_id in customer_ids:
            ...
            mappings_to_create = []
            for customer_frame in customer_frames:
                # Check for existing mapping before adding to the list
                if not CustomerOtherPostFrameMapping.objects.filter(
                    Q(customer_id=customer_id) & 
                    Q(other_post=instance) & 
                    Q(customer_frame=customer_frame)
                ).exists():
            ...
            if mappings_to_create:
                with transaction.atomic():
                    try:
                        ...
                        # Handle the case where a mapping might have been created by another process
                        pass
